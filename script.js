...
let reportesLayerGroup = L.layerGroup(); // Inicia apagada
...
// BUSCADOR DE FECHAS (Prioridad absoluta al campo 'date' YYYY-MM-DD)
...
function encontrarTipo(item) {
    const tipo = item.sv || item.tipo_reporte || item.tipo_incidente || item.tipo || 'Colisión Vehicular';
    return tipo;
}
...
function renderizarCapas(fechaInicio = null, fechaFin = null) {
    ...
    renderizarMarcadores(repPorFecha, reportesLayerGroup, {
        ...
        popupFormatter: (item) => {
            ...
            let fechaStr = fecha ? fecha.toLocaleDateString('es-EC') : 'Reciente';

            let numFallecidos = parseInt(item.Fallecidos || item.fallecidos || 0, 10);
            let numHeridos = parseInt(item.Heridos || item.heridos || 0, 10);
            let victimasStr = '';
            if(numFallecidos > 0) victimasStr += `<br><span style="color:red;">Fallecidos: ${numFallecidos}</span>`;
            if(numHeridos > 0) victimasStr += `<br><span style="color:orange;">Heridos: ${numHeridos}</span>`;

            return `<b>${tipoBonito}</b><br>
                    <span style="font-size:10px; color:#666">${fechaStr}</span>
                    ${victimasStr}<br>
                    ${item.descripcion || ''}`;
        }
    });
...
// -------------------------------------------------------------
// ESTADÍSTICAS (Ventana Deslizante y Gráficos)
// -------------------------------------------------------------
function actualizarDashboard(repFiltered) {
    ...
    rep.forEach(item => {
        ...
        const numFallecidos = parseInt(item.Fallecidos || item.fallecidos || 0, 10);
        const numHeridos = parseInt(item.Heridos || item.heridos || 0, 10);

        if (numFallecidos > 0) {
            countsByType['Fallecidos'] = (countsByType['Fallecidos'] || 0) + numFallecidos;
        }
        if (numHeridos > 0) {
            countsByType['Heridos'] = (countsByType['Heridos'] || 0) + numHeridos;
        }

        if (cleanType !== 'Atropello fallecido' && cleanType !== 'Atropello herido' && cleanType !== 'Fallecidos' && cleanType !== 'Heridos') {
    ...
    if (!isFiltered && sortedMonths.length > 3) {
        monthsToShow = sortedMonths.slice(-3);
    }
    ...
    chartInstanceType = new Chart(ctxType, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#ef4444', '#f97316', '#3b82f6', '#a855f7', '#22c55e', '#eab308'],
                borderWidth: 0
            }]
        },
        options: {
...
document.getElementById('reportForm').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        const res = await fetch(`${SUPABASE_URL}/rest/v1/reportes`, {
            ...
            body: JSON.stringify({ latitud: lat, longitud: lon, tipo_reporte: tipo, descripcion: desc, date: fechaActualDate })
        });
...
// -------------------------------------------------------------
// GENERACIÓN DE GEOJSON (DATOS ABIERTOS CON METADATOS)
// -------------------------------------------------------------
function descargarGeoJSON() {
    ...
    const geojson = {
        ...
        metadata: {
            ...
            descripcion: "El Geo Portal Vial Riobamba es una iniciativa que implementa información de incidentes de tránsito dentro del perímetro urbano de la ciudad de Riobamba, con la finalidad de recopilar, procesar, analizar, verificar y contrastar datos de incidentes para la prevención, mitigación de estos sucesos, poder identificar zonas de la ciudad con mayor densidad de choques, analizar patrones espaciales para la toma de decisiones.",
            metodologia: "Los archivos descargados son en formato GeoJSON para analizalo en software SIG. IMPORTANTE: Realizar una proyección al sistema UTM Zona 17 Sur (EPSG:32717) ya que los datos vienen por defecto en coordenadas geográficas WGS 84 (EPSG:4326). La generación de información se la generó por medio de recopilación bibliográfica de medios locales.",
            modo_de_uso: "Al descargar la información, y luego de realizar la proyección se encontrar campos relevantes en la tabla de atributos como; latitud y longitud que contienen la coordenada exacta del suceso. descripcion: contiene una breve descripcion del suceso como hora, fecha, heridos, falllecidos, etc. IMPORTANTE: para analisis espacial y temporal considerar los campos 'hora' y 'date'. Tambien contiene campo (numerico) 'Fallecidos' y 'Heridos'."
        },
        crs: {
            type: "name",
            properties: {
    ...
    a.download = `reportes_riobamba_segura_${fecha}.geojson`;
