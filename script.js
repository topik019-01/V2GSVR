...
function encontrarFecha(item) {
    // 1. Buscamos primero el campo específico 'date'
    if (item.date && typeof item.date === 'string' && item.date.length >= 10) {
        const parts = item.date.split('-');
        // Forzamos la creación de la fecha local a las 00:00:00 para evitar saltos de día/mes
        return new Date(parts[0], parts[1] - 1, parts[2]); 
    }

    // 2. Fallback por si algún registro no tiene la columna 'date' llena
    const fechaFallback = item.fecha_reporte || item.fecha || item.created_at;
    if (fechaFallback) {
        let fecha = new Date(fechaFallback);
        if (!isNaN(fecha)) {
            fecha.setTime(fecha.getTime() - (5 * 60 * 60 * 1000)); // Ajuste de -5h
            return fecha;
        }
    }
...
function renderizarCapas(fechaInicio = null, fechaFin = null) {
    ...
    // Filtrado Base por Fecha solo de la capa consolidada
    const repPorFecha = filtrarPorFecha(allReportesData, fechaInicio, fechaFin);

    // Reportes (Histórico y Ciudadano unificado)
...
function actualizarHeatmap(repData) {
    ...
    // Reportes Unificados (SOLO Colisión Vehicular)
    if (repData) {
...
function actualizarDashboard(repFiltered) {
    ...
    // 1. Contar por TIPO y sumar Fallecidos/Heridos
    const countsByType = {};
    ...
    // 2. Contar por MES utilizando la fecha exacta
    const countsByMonth = {};
    rep.forEach(item => {
    ...
    // LÓGICA: Mostrar solo los últimos 3 meses cronológicos si no hay filtro activo
...
document.getElementById('reportForm').addEventListener('submit', async function (e) {
    ...
    try {
        // Obtenemos la fecha actual en formato YYYY-MM-DD para insertarla correctamente
        const hoy = new Date();
        ...
        const res = await fetch(`${SUPABASE_URL}/rest/v1/reportes`, {
            ...
            // Aseguramos que el nuevo reporte también guarde el campo 'date'
...
function descargarGeoJSON() {
    ...
    allReportesData.forEach(item => {
        ...
        if (geom) {
            const feature = {
                ...
                geometry: {
                    type: "Point",
                    coordinates: [geom.lng, geom.lat] // GeoJSON requiere formato [longitud, latitud]
                },
                // Guardamos todas las columnas de Supabase íntegras como propiedades
                properties: { ...item }
            };
    ...
    // Convertimos el objeto GeoJSON a un texto (string) formateado
    ...
    // Creamos un enlace invisible, le asignamos el archivo y simulamos un clic para descargar
    const a = document.createElement('a');
    a.href = url;
    
    // Asignamos el nombre del archivo con la fecha de hoy
