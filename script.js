...
function renderizarCapas(fechaInicio = null, fechaFin = null) {
    ...
    // Capturar valor del filtro de tipo (Para los marcadores visuales)
    const tipoFiltroElement = document.getElementById('filterType');
    const tipoFiltro = tipoFiltroElement ? tipoFiltroElement.value : 'todos';

    // 1. Filtrado Base por Fecha (Común para todo)
    ...
    // 2. Preparar Datos para MARCADORES (Respetan Filtro de Tipo)
    let repParaMarcadores = repPorFecha;
    
    if (tipoFiltro !== 'todos') {
        // Filtramos la lista de reportes para mostrar solo lo que el usuario eligió ver
        repParaMarcadores = repPorFecha.filter(item => 
            normalizarTexto(encontrarTipo(item)) === normalizarTexto(tipoFiltro)
        );
    }

    // 3. Renderizar Marcadores
    // Accidentes (Capa Histórica - Siempre visible si está en fecha)
    ...
    // Reportes (Capa Ciudadana - Visible según filtros)
    renderizarMarcadores(repParaMarcadores, reportesLayerGroup, {
        radius: 7,
        color: "#ffa500", 
        fillOpacity: 0.8,
        popupFormatter: (item) => {
    ...
    // 4. Actualizar Heatmap (Lógica Especial: Solo Colisiones)
    // Pasamos los datos filtrados por fecha, pero la función filtrará por tipo "Colisión" internamente
    // Esto es independiente de los marcadores que estés viendo.
    actualizarHeatmap(accPorFecha, repPorFecha);

    // 5. Dashboard (Refleja lo que se ve en el mapa)
    actualizarDashboard(accPorFecha, repParaMarcadores);
}
...
function actualizarHeatmap(accData, repData) {
    ...
    // 1. Accidentes Históricos (Filtramos solo Colisiones Vehiculares para ser precisos)
    // O si prefieres que TODOS los accidentes históricos cuenten, borra el 'if' dentro del loop.
    if (accData) {
        accData.forEach(item => {
            let tipo = normalizarTexto(encontrarTipo(item));
            // Asumimos que queremos ver mapa de calor de CHOQUES.
            // Si la base histórica tiene "Atropellos", ¿los incluimos?
            // Tu pedido: "Solo se aplique para colision vehicular".
            if (tipo === 'Colision vehicular') {
                let g = parseGeom(item);
                if(g) puntos.push([g.lat, g.lng, 1.0]); // Peso 1
            }
        });
    }
    ...
    if (puntos.length > 0 && typeof L.heatLayer === 'function') {
        heatmapLayer = L.heatLayer(puntos, {
            radius: 40,      // Radio amplio para conectar puntos cercanos
            blur: 25,        // Difuminado suave
            maxZoom: 15,
            
            // LÓGICA DE INTENSIDAD (1=Azul, 3=Rojo)
            max: 3.0,        // Techo de calor
            minOpacity: 0.5, // Hace que 1 solo punto sea visible y sólido
            
            gradient: { 
                0.3: 'blue',   // 1 incidente
                0.6: 'yellow', // 2 incidentes
                1.0: 'red'     // 3+ incidentes
            }
        });
        
        // Solo añadir si el toggle está activo
...
function actualizarDashboard(accFiltered, repFiltered) {
    ...
    chartInstanceTime = new Chart(ctxTime, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Incidentes',
                data: sortedMonths.map(m => countsByMonth[m]),
...
async function init() {
    ...
    perimetroData.forEach(item => {
        if (item.geom) {
            ...
            // 2. Guardar en memoria para validar
...
// -------------------------------------------------------------
// ENVÍO DE FORMULARIO (VALIDA UBICACIÓN)
// -------------------------------------------------------------
document.getElementById('reportForm').addEventListener('submit', async function (e) {
    ...
    // VALIDACIÓN DE SEGURIDAD: PERÍMETRO URBANO
...
// MOBILE FIX: Add click listener to the panel handle
